################################################################################
### Plot the SPC 2x2 Categorical Risk map for Tennessee                      ###
...
# Convert the bounding box coordinates from WSG84 to crs_proj
bbox <- 
  st_sfc(st_point(c(map_x_min, map_y_max)), st_point(c(map_x_max, map_y_min)), crs = "EPSG:4326") %>%
  st_transform(., crs = crs_proj) %>%
  st_coordinates()

# Function to pull the outlooks maps from the NWS SPC website
...
# Convert that to POSIXlt datetime.
...
map_states <- 
  ...
  st_transform(crs = crs_proj)
map_counties <- tigris::counties() %>% filter(STATEFP == "47") %>% st_transform(crs = crs_proj)
...
graph_spc_map <- function(map_type, map_title, map_legend_title, map_legend_levels) {
  ...
  geom_sf(data = map_counties, fill = NA, color = "black", linewidth = 0.3) +
  
  # Bounding box for TN
  coord_sf(xlim = bbox[,'X'], ylim = bbox[,'Y'], datum = crs_proj, expand = FALSE) +
...
# Render the final graph using the "patchwork" library
